...
double monte_carlo(const array<uint8_t,2>& me, const vector<array<uint8_t,2>> &opponent, uint64_t board = 0, uint8_t last = 0, double err = 1, int min_iters = 100, auto duration = chrono::minutes(1), double threshold = NAN, uint64_t dead = 0) {
    ...
    do {
        ...
        confidence = 1.96 * stdev / sqrt(ntrials);
            ...
            ntrials >= min_iters &&
            (confidence <= err || abs(p - threshold) > confidence)
            ));
    return p;
}
...
template<bool from_scratch = false>
double callEv(int rounds_wo_all_in, array<uint8_t,2> hand, vector<uint8_t> board, auto duration) {
    ...
    if (!rounds_wo_all_in && !from_scratch) {
        assert(preflop.count(hand_rep(hand)));
        return preflop[hand_rep(hand)];
    }
    double lg = pow(0.2, rounds_wo_all_in) * 0.8, lb = pow(0.8, rounds_wo_all_in) * 0.2;

    array<vector<array<uint8_t,2>>,2> op_hands;
    for (auto x : hands_except(dead))
        op_hands[is_blind_bandit_good(x)].push_back(x);
    ...
    assert(0 <= equities[1] && equities[1] <= 1);
    return -100 + 200 * (prob_good * equities[1] + (1 - prob_good) * equities[0]);
}

double callEv(int rounds_wo_all_in, array<string,2> hand_string, vector<string> board_string, auto duration) {
    ...
    return callEv(rounds_wo_all_in, hand, board, duration);
}
...
int main() {
    ...
    for (int i = 0; i < 13; i++) {
        ...
        for (int j = 0; j < 13; j++) {
            ...
            hand = hand_rep(hand);
            cout << "{array<uint8_t,2>{" << int(hand[0]) << ", " << int(hand[1]) << "}, " << preflop[hand] << "}," << flush;
        }
    }
    cout << "};";
}
